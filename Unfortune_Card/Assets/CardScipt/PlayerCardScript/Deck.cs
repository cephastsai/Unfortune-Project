...
public class Deck : MonoBehaviour {
	...
	public void Drawing(PlayerCardManager.MainSection Tsec){		
									
		if(DeckList.Count +Deadwood.Ins.DeadwoodList.Count ==0){			
			PlayerCardManager.Ins.MainSectionQue.Dequeue();
		}else if(DeckList.Count == 0){
			Tsec.MSection = CardManager.cardSection.Shuffle;
			PlayerCardManager.Ins.AddMainQue(CardManager.cardSection.Drawing);
			PlayerCardManager.Ins.SectionStart();
